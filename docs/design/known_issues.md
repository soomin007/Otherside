...
- **증상:** 뷰포트가 좁아질 때(글자 배율 >100%·작은 창·에디터 임베드뷰) 일지 왼쪽 페이지 내용이 책등을 넘거나 재배치를 여러 번 해야 서서히 줄어듦. (2026-07-15 규명 — 07-07 "글자 크기 되돌림"의 실제 원인)
  **원인:** ScrollContainer **축소 랫칫**. `Control.set_size` 는 최소 크기 밑으로 못 줄이는데, 자식(`_box_l`)의 `custom_minimum_size.x` 를 "대입된 scroll.size" 에서 파생시키면 순환이 생긴다 — scroll 은 자식 최소폭에 막혀 못 줄고, 자식 최소폭은 그 막힌 값에서 -20 만 줄어 재배치마다 20px 씩만 수렴.
  **재발 방지:** 컨테이너를 줄일 땐 **자식 최소 크기를 목표값 기준으로 먼저** 줄이고 나서 부모 size 를 대입한다(대입값을 다시 읽어 파생하지 말 것). 커질 때는 안 걸려서 조용히 숨는다 — 축소 방향 테스트를 꼭 포함.

- **증상:** 글자 배율(>100%)에서 일지 오른쪽 챕터 탭이 화면 밖으로 잘림. (2026-07-15)
  **원인:** 책 폭 상한 1080 은 기준 화면(1280)에서 "가운데 정렬 후 오른쪽 여백 100 = 탭 폭"이 정확히 성립하는 값이었다. 논리 뷰포트가 좁아지면 여백이 탭 폭보다 작아진다.
  **재발 방지:** `Bookmark._layout_book` 폭 상한에 `vp.x - 200` 조건 추가(탭 예약폭). 화면 가장자리에 딱 맞춘 고정 상한은 뷰포트가 줄면 깨진다 — 상한은 항상 뷰포트 상대 조건과 함께.

- **증상:** 화면 배율 >100% 에서 배낭 챙기기 하단 줄이 양끝 버튼(기본 채비·떠난다)을 침범. 고치려고 줄에 건 `scale` 축소가 값은 찍히는데 화면엔 적용이 안 됨(다음 프레임에 1로 복귀). (2026-07-16)
  **원인:** 두 겹. ① 중앙 줄(~810px)이 배율로 좁아진 논리 뷰포트(120% 데스크톱 = 1067px)에서 코너 고정 버튼과 겹침. ② Godot `Container.fit_child_in_rect` 가 재정렬(리사이즈·자식 min 변화)마다 **직속 자식의 scale·rotation 을 리셋** — CenterContainer 자식에 scale 을 걸면 소리 없이 원복된다.
  **재발 방지:** Container 직속 자식에는 scale 을 걸지 않는다. 스케일이 필요한 UI 는 평범한 Control 아래로 빼 위치·크기·pivot 을 직접 잡는다(`Loadout._fit_bag_row`). "가운데 묶음 + 코너 고정" 배치는 배율 120%(뷰포트 축소) 스크린샷으로 겹침 검사까지.
...
- **증상:** 웹에서 붓글씨(나눔손글씨) 표제의 **가운뎃점(·)이 두부(□)**. 데스크톱은 정상. (2026-07-05, 일지 설정 챕터 "화면 · 이야기")
  ...
  **후기(2026-07-19):** 붓 폰트 자체를 폐지(사용자 "싸 보인다") — 폰트 파일 삭제, 전 화면 명조 통일. 이 항목의 폴백 장치도 함께 제거됐다. 교훈(임베드 폰트 글리프 확인)은 유효 — 새 폰트를 들일 때 같은 함정 주의.
...
- **증상(프로세스 실수):** `ACTIVE_WORK.md` 블록에 실제 새벽 1시인데 "시작 03:05·03:40, ETA 04:10" 같은 지어낸 시각을 적었다(2026-07-18 사용자 지적). 조율판의 시각이 틀리면 다른 세션의 "ETA 한참 지남 = 중단 추정" 판정과 대기 판단이 전부 어긋난다.
  **원인:** 시스템 시계를 확인하지 않고 직전 블록들의 시각에서 관성으로 이어 적었다(앞 블록들이 밀리면 뒤도 같이 밀림).
  **방지:** ACTIVE_WORK 블록을 쓰기 전에 **반드시 `Get-Date` 로 실제 시각을 확인**하고 그 값으로 시작·ETA 를 적는다. 추정·이어붙이기 금지.

- **증상:** 데스크톱(마우스)에서 지도의 다음 행선지 **호버 원이 안 그려짐**(클릭·탭은 정상). (2026-07-18 사용자 제보)
  **원인:** 2026-07-12 터치 이중입력 가드가 **기기 종류로** 걸렀다 — `is_touchscreen_available()` 이 true 인 기기에서 마우스 이벤트 전부 무시. 이 판별은 "터치 가능"과 "터치 사용 중"을 구분 못 해, 터치 가능 데스크톱(이 개발 PC 포함)에서 **진짜 마우스 모션까지 버려** 호버가 죽었다. 탭은 `emulate_touch_from_mouse` 가 합성한 ScreenTouch 로 우연히 살아 있어 클릭만 되는 것처럼 보였다.
  **방지:** 입력 중복 제거는 기기 종류가 아니라 **이벤트의 출신**으로 거른다 — 합성본은 `event.device == InputEvent.DEVICE_ID_EMULATION`. (양방향 커버: 데스크톱 클릭→합성 ScreenTouch, 폰 터치→합성 마우스.) 폰 두루마리 토글은 재확인 목록에 올려 회귀를 본다.

- **증상:** 지도에서 노드(모래의 벽)를 클릭해도 **자꾸 옆의 흔적 표식만 열림**. (2026-07-18 사용자 제보)
  **원인:** 탭 처리 순서가 표식 먼저 + 표식 판정 반경이 넉넉(데스크톱 28px·터치 38px)해, 노드 왼쪽에 쌓이는 표식 아이콘의 판정이 **노드 자체 판정(중심 0.62×크기) 안쪽까지 침범**해 탭을 가로챘다(실측: 중심 −0.3~−0.62×크기 구간).
  **방지:** 겹침 중재를 한 함수(`Map._pick_target`)로 — **노드 우선, 아이콘 몸통 정타(≈14px)만 표식 인정.** 탭과 호버가 같은 규칙을 쓴다. 히트박스가 겹치는 UI 를 만들면 "무엇이 이기나"를 명시적으로 정할 것.

- **증상:** 폰 itch 에서 **뒤로가기로 전체화면을 나갔다가 "Restore game" 으로 복귀하면 화면은 돌아오는데 터치가 전부 먹통**. PC 의 Esc → 클릭 복귀는 정상. (2026-07-26 사용자 제보, 베타 0.3)
  **원인(후보 2건, 0.3.1 에서 둘 다 방어):** ① 안드로이드 뒤로가기 = 가장자리 스와이프 **터치 제스처**라 touchend 없이 시스템이 가로채면 엔진에 "눌린 손가락"이 남고, 이후 모든 탭이 두 번째 손가락 취급(마우스 에뮬레이션은 0번만 따라감 → UI 전체 무반응). ② 폰 itch 는 래퍼가 전체화면을 소유하는데 우리 `Fullscreen._input` 이 매 탭 캔버스로 전체화면을 재요청 — 소유권을 뺏으면 래퍼가 이탈로 오판해 오버레이를 덮을 수 있음.
  **방지(Fullscreen.gd):** 포커스 복귀·뷰포트 리사이즈마다 **터치 index 0~9 를 release 로 주입**해 고착 손가락 해제(안 눌린 index 는 no-op). **iframe + 터치 기기에선 전체화면 요청을 생략**(호스트 소유 존중, 가로 잠금 JS 는 유지 — 누가 전체화면이든 fullscreenchange 에 걸림). 재발하면 Pages 판(래퍼 없음)에서 같은 절차 재현으로 우리/itch 쪽을 분리 진단.

- **증상(0.3.3 까지 지속, 상세 제보로 특정):** 폰 itch 에서 **일지가 열린 채** 뒤로가기 → Restore 하면 복귀 순간 페이지 넘김 모션이 저절로 나오고 차례/언어 면이 열린 뒤 전부 먹통. 챕터 책갈피도 사라져 보임. (2026-07-27)
  **원인(기전):** 안드로이드 뒤로가기 = **오른쪽 가장자리 스와이프 터치**인데, 일지의 챕터 탭 띠가 정확히 책 오른편~화면 끝 영역이라 스와이프의 "누름"이 탭에 얹힌다. 탭·스크림이 **press 기준**이라 그 즉시 챕터 넘김(`_flip_to`)이 시작되고, `_run_flip` 의 `await RenderingServer.frame_post_draw` 는 화면이 숨겨지면(웹 rAF 정지) 그대로 매달린다. `_flipping=true` 로 굳은 채 복귀하면 밀린 넘김이 재생되고(제보의 "저절로 넘김 모션"), 뗌 이벤트는 시스템이 가로채 영영 안 와 입력 상태가 꼬인다.
  **방지(0.3.4):** ① 탭·스크림을 **뗄 때(release) 기준**으로 — 뗌이 안 오는 시스템 제스처는 아무 것도 시작 못 한다(탭 밖에서 떼면 취소. Button 류는 원래 release 기준이라 무관). ② `_run_flip` 에 **워치독 2초** — 매달린 넘김을 복귀 후 자동 강제 정리(`_force_end_flip`: 덮개 제거·재배치·재구축). ③ 교훈: **웹에서 `await frame_post_draw` 는 화면이 숨겨지면 안 돌아온다** — 이 await 뒤에 상태 잠금(_flipping 류)이 걸려 있으면 반드시 워치독을 함께 둘 것. 커스텀 gui_input 핸들러는 press 가 아니라 release 기준이 기본값.

- **경과(0.3.4 로도 미해결 — 진행 중, 2026-07-27):** 증상 동일(복귀 시 넘김 모션·차례 랜딩·책갈피 사라짐·전체 무반응). **프로브 판독이 결정적**: 죽은 상태의 모든 터치가 `ev: ScreenDrag` 로 들어옴 = 입력은 엔진에 도달하지만 새 터치가 "이미 눌린 손가락의 이동"으로 해석된다. **고착 터치가 GDScript 가 아니라 엔진의 브라우저 입력 층(JS/DisplayServer 내부 활성 터치 목록)에 잔존** — `Input.parse_input_event` 로 주입한 release 는 그 층을 청소하지 못한다(터치스타트가 위 어딘가에서 먹히고 move 만 전역 리스너로 새어 들어오는 그림도 합치).
  **다음 시도(내일):** ① **유령 드래그 재합성** — Fullscreen._input 에서 "눌림 확인 안 된 index 의 ScreenDrag" 를 감지하면 그 자리에서 ScreenTouch dn 을 합성하고, 이후 드래그는 그대로 흘리다 ~180ms 이벤트 공백 시 up 합성(탭·드래그 모두 복원, 정상 스트림엔 무개입). ② JS 층 직접 청소 — canvas/document 에 pointercancel·touchcancel 합성 dispatch. ③ 프로브 버전 필드로 0.3.4 반영 여부 먼저 확인(fl/dk/ch/sp 값도 — 0.3.4 인데도 넘김 모션이 난다면 별도 기전).

- **경과(0.3.5 구현 — 폰 재검증 대기, 2026-07-27):** 위 설계 ①②를 그대로 구현. `Fullscreen.gd` 에 `_track_ghost`(눌린 index 집합 추적, 눌림 없는 ScreenDrag → 그 자리 dn 합성) + `_sweep_ghosts`(드래그 180ms 공백 시 마지막 위치에 up 합성, `_process` 에서 상시) + 복귀 치유에 `TOUCH_CANCEL_JS`(canvas 에 touchcancel 0~9·pointercancel dispatch). 진단 줄에 드래그 index(`ScreenDrag#n`)와 재합성 횟수(`gh:n`) 추가. 검증: 창모드 드라이버 7항목 전부 통과(유령 등록·dn 중복 없음·스윕 후 버튼 클릭 완성·정상 스트림 무개입·진짜 up 오면 자연 종료), 부팅 스모크 0 에러(종료 누수 경고는 HEAD 에도 있는 기존 노이즈), 세이브·설정 해시 불변. **판정법(폰):** 죽던 절차 후 진단 줄에서 `gh` 가 오르며 터치가 살아나면 재합성이 정답. `gh:0` 인데 살아나면 JS 청소가 정답. 둘 다인데도 죽으면 `ev` 가 `ScreenDrag` 외 무엇으로 오는지 관찰.

- **경과(0.3.5 도 미해결 — 폰 확인, 2026-07-27 아침):** 사용자 폰 확인(v0.3.5 반영 확정): 복귀하면 이전 화면 그대로처럼 보이다가 **첫 터치에서** 설정 차례로 넘어가고 책갈피가 사라지며 전부 먹통. 프로브: 대기 중 마지막 ev 가 `up`(+타이머 증가), 터치 중엔 `ScreenDrag`. **판독:** ① 뗌(touchend)은 엔진에 도달한다. ② 움직임 없는 깨끗한 탭은 **드래그 없이 뗌만** 온다 — 0.3.5 재합성은 드래그에만 반응해 이 경우를 놓친다. ③ `_on_tab_input` 의 release 기준엔 **"그 탭을 먼저 눌렀는가" 확인이 없어**, 누름 없는 유령 뗌이 탭 위에 떨어지면 넘김이 그대로 발동(첫 터치 = 설정 차례 오발의 직접 원인). 스크림 닫기도 같은 구멍.
  **방지(0.3.6):** ① Fullscreen._track_ghost 에 **유령 뗌 재합성** — 누름 없는 ScreenTouch up 을 `set_input_as_handled()` 로 삼키고 그 자리에 dn+up 짝을 합성(치유 주입분 위치 (-1,-1) 은 제외 — 재귀·gh 오염 방지). 이것으로 release 기준 커스텀 핸들러 전체가 유령 뗌으로부터 보호된다. ② 겹벨트: 탭(`_tab_armed`)·스크림(`_scrim_armed`)에 **누름 선행(arming) 요구** — 같은 컨트롤 위 누름이 확인된 뗌만 유효. ③ 넘김 **폴링 워치독**(Bookmark._process, 2.5초) — 타이머 콜백이 어떤 이유로든 안 와도 매달린 넘김을 정리(책갈피 사라짐 고착 방어). 교훈: **release 기준 커스텀 핸들러는 반드시 "누름 선행" 확인까지 한 쌍이다** — 뗌만 보는 핸들러는 시스템 제스처·유령 터치가 오발시킨다.

- **경과(0.3.5 프로브 정밀 판독 → 0.3.7, 2026-07-27 오후):** 사용자 폰 프로브 값(죽은 상태): p:0 · heal:5 · gh:0 · fl:0 · dk:0 · ch:3 · sp:0 · rw:0. 왕복마다 heal +4, gh 는 드물게 1. **판독:** ① gh:0 = 죽은 상태에서도 드래그의 index 가 "눌림 확인됨" — **누름(dn)이 실제로 도착한다.** 입력 스트림은 온전하고, 문제는 그 뒤 층이다. ② 그렇다면 뒤로가기 스와이프는 "누름은 탭 위 + 시스템 가로챔 후 **touchcancel** = canceled 뗌이 탭 위 좌표로 도착"이 된다 — **canceled 뗌을 탭 핸들러가 진짜 뗌으로 오인**하면 arming(0.3.6)까지 통과해 설정 넘김이 발동한다(0.3.4 의 "release 기준" 전제였던 "뗌이 안 온다"가 틀렸던 것). 0.3.6 의 유령 뗌 재합성도 canceled 를 구분 안 하면 **뒤로가기마다 가짜 탭을 만드는 역효과**가 된다(배포 전 발견). ③ p:0 = **일지 열림 + 일시정지 풀림**, 정상 경로에 없는 상태(_close 만 풀며 동시에 패널을 숨긴다) — 경로 미상, 죽음의 "GUI 무반응" 층과 관련 의심. ④ fl/dk:0 = 넘김 고착 아님.
  **방지(0.3.7):** ① `InputEventScreenTouch.canceled` 를 탭(`_on_tab_input`)·스크림(`_on_scrim_input`)에서 무효 처리(disarm 만) + Fullscreen 유령 뗌 재합성에서 제외. ② "일지 열림 ⇒ 일시정지" **불변식 자가 수리**(Bookmark._process, 되잠금 + rp 카운터). ③ 프로브에 cx(취소 뗌 표시)·tb(Transition.busy)·rp 추가 — 다음 폰 관찰에서 "죽은 탭의 ev 가 up 인가 cx 인가"(취소 가설 확정), "rp 가 오르는가"(pause 풀림 빈도), "tb:1 인가"(전환 고착) 판별. 교훈: **터치 핸들러는 pressed/released 두 갈래가 아니라 canceled 까지 세 갈래다** — 시스템 제스처(뒤로가기)는 canceled 로 끝나며, 이를 뗌으로 처리하면 가짜 탭이 된다.

- **원인 특정(0.3.7 폰 스크린샷 2장 → 0.3.8, 2026-07-27 저녁):** 전/후 프로브 — 전: p:1·heal:1·gh:10·ch:0(정상). 후(죽음): p:1·heal:6·**gh:40**·tb:0·fl:0·dk:0·**ch:3**·rp:0, 책갈피가 화면에서 실제로 사라짐, ev:MouseButton(+6.3s, 치유 주입분). **판독과 확정:** ① gh 30 회 발동에 전원 무반응 = **합성 이벤트 좌표가 틀렸다.** `Input.parse_input_event` 재주입은 이벤트를 "창 좌표"로 받아 **스트레치 변환을 다시 적용**한다 — `_input` 에서 본 좌표(뷰포트 로컬)를 그대로 주입하면 이중 변환(폰: 창 2340x1080 → 뷰포트 1509x720, x0.65 두 번)으로 좌상단행. **진짜 누름은 제자리 + 합성 뗌은 딴 곳 → 모든 탭이 "누르고 떼지 않음" = 전체 무반응.** 0.3.5 부터의 자가 치유(스윕 뗌)가 폰 먹통을 오히려 완성하고 있었다. 데스크톱 검증은 창=뷰포트 1:1(변환 항등)이라 전부 통과 — 함정을 못 잡았다. ② ch:3 재발 = 스와이프가 남긴 넘김 코루틴이 **복귀 첫 렌더의 frame_post_draw 에서 깨어나 스왑을 완주**(외출 2.5초 미만이면 폴링 워치독 문턱 미달, 2초 SceneTreeTimer 는 process 시간이라 더 늦음). ③ cx 미출현 = 웹 플랫폼이 touchcancel 을 touchend 로 접어 보낼 가능성(canceled 무용). ④ p:0(이전 관찰)은 이번엔 재현 안 됨(rp:0).
  **방지(0.3.8):** ① 합성 이벤트 주입 전 `get_viewport().get_final_transform() * pos` 로 **창 좌표로 되돌린다**(`Fullscreen._to_window`, 유령 dn·스윕 up·뗌 짝 전부). ② `_run_flip` 이 await 전후 시계를 재서 **1초 이상 지났으면(숨김 통과) 스왑 없이 중단** — 챕터 보존, "복귀하면 저절로 설정 차례" 차단. ③ 복귀 치유에 책갈피 강제 표시+redraw 보험(사라짐 기전 미상— 상류 차단으로 함께 소멸 기대). 검증: **창 1890x1080(스케일 1.5, 폰 동일 조건)** 드라이버로 합성 탭 클릭 완성 확인. 교훈 둘: **재주입 좌표는 창 좌표계다** — 뷰포트 스케일 환경(웹 폰)에서 로컬 좌표 재주입은 이중 변환된다. **입력 검증 드라이버는 반드시 창≠뷰포트(스케일≠1) 환경으로도 돌린다** — 1:1 창은 좌표계 버그를 전부 통과시킨다.

- **해결(0.3.10 폰 확인, 2026-07-27 밤):** 0.3.9 계측(tv/nv)이 정체를 지목 — **챕터 탭 4개가 화면 원점 (0,0)으로 이탈**(살아 있고 visible, 좌표만 이탈)한 채 히트 영역이 터치를 전부 삼켰다. 첫 터치 = 겹친 스택 맨 위(설정 탭) → ch:3 넘김, 이후 터치는 같은 챕터라 `_flip_to` 가 무시 = "전부 먹통". **방지(0.3.10):** `_layout_book` 탭 루프가 앵커(`set_anchors_preset(TOP_LEFT)`)·크기·자리를 **매번 강제로 되돌린다**(이탈 경로 불문, 복귀 치유마다 실행). 폰 재검증으로 해결 확인(tv:VVVV@1290,86 복원). (0,0) 이탈을 만드는 근본 경로는 미상으로 남음 — t0 계측(좌표/크기/부모/트리)을 프로브에 유지, 재발 시 그 값으로 추적. 교훈: **오버레이 먹통은 "안 보이는데 입력은 받는" 컨트롤부터 의심** — visible/valid/좌표를 한 번에 찍는 계측이 가설 세 번보다 빨랐다.

- **증상:** 폰에서 원정대 이름칸을 누르면 **가상 키보드가 뜨자마자 닫혀** 입력 불가. (2026-07-27 사용자 제보, 0.3.10)
  **원인:** 가상 키보드가 열리면 브라우저가 캔버스를 **리사이즈**한다 → 리사이즈마다 도는 복귀 치유(`_refresh_hint` → `_release_stuck_touches`)가 `gui_release_focus()` 로 이름칸 포커스를 지움 → 키보드 즉시 닫힘. 자기가 연 키보드를 자기 치유가 닫는 자충수.
  **방지(0.3.11):** 치유에 `clear_focus` 매개변수 — **리사이즈 경로는 포커스를 보존**(고착 터치 해제만), 포커스 정리는 복귀(포커스 인·전체화면 전환) 경로에서만. 교훈: 리사이즈 신호는 "복귀"만이 아니라 **가상 키보드·브라우저 바 변화로도 온다** — 리사이즈에 건 훅은 키보드 시나리오를 반드시 따져 본다.
  **경과(0.3.11 로도 닫힘 → 0.3.12):** 포커스 보존만으론 부족(잔여 치유 동작 — JS touchcancel 청소 등 — 가 키보드를 닫는 것으로 추정. 웹 가상 키보드 = 숨은 HTML input 의 DOM 포커스라 canvas 쪽 개입에 취약). **글자 입력 중(LineEdit/TextEdit 포커스)에는 리사이즈 치유를 통째로 건너뛴다** — 어느 하위 동작이 범인이든 원천 차단. 복귀 경로는 전체 치유 유지.

- **증상:** 데스크톱(1280 창)에서 챕터 탭이 **화면 오른쪽 끝에서 잘리고**, 한국어로 바꿔도 **탭 글씨가 영어로 남음**. (2026-07-27 사용자 제보)
  **원인:** ① 책이 화면 중앙 고정이라 오른쪽 여백이 (vp.x-w)/2 = 104px 뿐인데 영어 탭 길이는 ~170px(한국어도 빠듯). 여백 가정이 한국어 짧은 챕터명 기준이었다. ② 탭 생성 때 `text = L10N.t(원문)` 으로 **번역 결과를 저장** — 그리기 시점 재번역이 "Chronicle" 을 키로 찾다 실패해 영어가 고정(이중 번역).
  **방지(0.3.11):** ① `_layout_book` 이 현재 언어의 탭 필요 길이(`_tab_base_len`)를 재서 **책을 왼쪽으로 민다**(중앙 고정 폐기, 언어 전환 시에도 재배치). ② 표시 문자열은 **원문(한국어 키)을 저장**하고 번역은 표시 순간에만 — `t()` 결과를 저장하면 언어 전환이 영영 안 먹는다(L10N 원칙 재확인).
