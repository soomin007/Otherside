...
- **증상:** 단면 지점 라벨이 화면 중앙 하단의 안내 문구("표시된 곳을 눌러 살핀다" 등, `Expedition._draw` y=rect.y-140 가운데 정렬)와 글자끼리 겹침. (2026-07-15 폰: 오아시스 "웅크린 사람")
  **원인:** 지점 라벨(마커 밑 y+r+20)과 안내 문구가 같은 화면 공간을 쓰는데 서로를 모른다. 중앙 x 대역 × y 0.71~0.78 부근의 지점이 정확히 그 창에 라벨을 떨어뜨린다.
  **재발 방지:** 지점·메인 `at` 은 **x (0.36,0.64) × y (0.705,0.785) 창을 피한다.** `core_smoke` 의 `_test_spot_coordinates` 가 이 창 침범도 잡는다(안내 y·폰트가 바뀌면 테스트 상수도 같이).

- **증상:** 긴 지점 라벨("앞선 이의 유품")의 양끝 글자가 안 읽힘 + 어두운 그림(독 웅덩이 진흙) 위 라벨 전반이 흐릿. (2026-07-15 폰)
  **원인:** 라벨 크림 웅덩이가 고정폭 156px 라 긴 라벨을 못 받치고, 반경 0.5 지점부터 페이드라 글자 끝이 알파 ~0.3 위에 앉음.
  **재발 방지:** `SectionArt.draw_spot` 이 웅덩이 폭을 글자 폭 비례(`max(140, tw*1.8)`)로 잡고 평탄 구간(0~0.62 알파 0.88→0.60)을 두도록 고침. 라벨을 바꾸면 스크린샷으로 실제 렌더 확인(특히 어두운 그림·긴 라벨).
