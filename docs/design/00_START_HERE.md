...
3. **결말 — 순환과 재회** (구현됨, 사용자 확정):
   ...
   - **출구 = 재회(따뜻, 진짜 엔딩).** 도달 자체는 순환과 같다(살아서 닿음) — 가르는 건 두 축(2026-07-09): ① 남긴 흔적 ≥ 4(`REUNION_TRACES`) ② **기린 죽은 자리 ≥ 2**(`REUNION_MOURN`, 추모=죽은 자리에 표식 남기기·런당 1회의 남기기 사용). 채우고 닿으면 먼저 간 모든 원정대와 함께 건너편에. 릴레이 멈춤.
...
- **⑥ 결말:** **엔딩 슬라이드쇼 구현됨**(`Ending.gd` 오버레이, 순환/재회 47~52 아트+음악, 2026-07-05, 둘 다 타이틀 복귀). **재회 암시 = 시장 옛말(순환 볼 때마다 반복, `reunion_hints_shown`) + 순환 암전 독백(`Ending._hint` "다시 한 번, 완벽한 원정을…")**(2026-07-09 — 남김+기림을 사람 말로, 숫자 비공개). 남은 것: 순환의 *물리적* 반영(도달점=다음 목표=맵 변화, 설계 큼), 재회 임계(4+기림 2) 폰 체감, 엔딩 폴백(기록 버튼 숨김·순환 배경곡·실플레이).
