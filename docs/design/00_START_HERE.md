...
9. **재회 카운트 = 의도적으로 남긴 것만(죽음 ≠ 재회 진행).** `player_trace_count`는 `traces`의 비-seed 항목만 센다(유령 제외). 죽음은 `deaths` 배열이라 카운트 0 — **시체·죽은 자리는 재회에 1도 기여 안 함**(정보·정서 전용, `TraceData` 주석 "정보 0 정서 100"). 재회는 오직 *남기기*로 연다(기획서 §3 주제: 죽음이 아니라 남김이 재회를 연다). 외부 테스트 공략이 "시체도 카운트"로 착각했던 지점(2026-07-09 임계 8→4 + 공략 정정).
