...
- **⑥ 결말:** **엔딩 슬라이드쇼 구현됨**(`Ending.gd` 오버레이, 순환/재회 47~52 아트+음악, 2026-07-05, 둘 다 타이틀 복귀). **재회 암시 = 첫 순환 후 시장 옛말**(`Loadout.REUNION_HINT_PAGES`·`reunion_hint_seen`, 2026-07-09 — 남김+기림을 사람 말로, 숫자 비공개). 남은 것: 순환의 *물리적* 반영(도달점=다음 목표=맵 변화, 설계 큼), 재회 임계(4+기림 2) 폰 체감, 엔딩 폴백(기록 버튼 숨김·순환 배경곡·실플레이).
