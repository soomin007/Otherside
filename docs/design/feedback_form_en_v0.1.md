...
여기 초안을 복사해 붙인다. **개설되면 URL 을 이 파일 상단과 itch 페이지 소개문에 기입한다.**

- 개설 전: itch 페이지에는 링크를 넣지 않는다(빈 링크 금지). 댓글란이 기본 창구.
- 개설 후: itch 페이지 소개문의 "Feedback" 줄에 링크 추가 + 이 파일 상단에 URL 기록.
