...
## 현재 상태 (2026-07-06)
- **완료:** C 오프닝 · D 가방(직능·도구·무게) · E 코어 ⑥ 결말(엔딩 슬라이드쇼) · **실제 아트 삽입 완료**(01~52 배선) · F 설정(원정 일지 챕터 통합·음량 저장) · **오디오**(BGM 4곡·SFX 29종 제작·배선) · **가로 고정** · **책장 넘김·씬 등장 stagger** · **지도 렌더 개편**.
- **진행/부분:** A 콘텐츠(계속 확충 여지) · B 비주얼 미세 다듬기(단면 맞춤 아트·지도 자원 점 범례) · E 결말 심화(순환 물리 반영·재회 임계 밸런싱).
- **지금 병행하기 좋은 것:** A(콘텐츠) · F(접근성·연출 속도) · 폭풍 3층(`StormFX`) · 위치 반영 환경음(오디오 배선 완료로 착수 가능). 대부분 폰 실플레이 정성 확인 단계.
...
### F. 설정·시스템 (독립)
- 범위: `scripts/ui/Bookmark.gd`(원정 일지 "설정" 챕터 — 옛 SettingsPanel 폐지), `scripts/AppSettings.gd`.
