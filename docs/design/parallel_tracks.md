...
## 현재 상태 (2026-07-01)
- **완료:** C 오프닝(서사 + 시장 NPC 대사) · D 가방 꾸리기 · E 코어 ⑥ 결말 MVP.
- **진행/부분:** A 콘텐츠(9노드 spots·이벤트 풀 배치됨, 계속 확충 여지) · E 결말 심화(순환 물리 반영·재회 밸런싱 남음).
- **미착수:** B 비주얼(단면 맞춤 아트·지도 다듬기·폭풍 파티클) · F 설정 확장.
→ 지금 병행하기 좋은 것: **B(비주얼) + A(콘텐츠) + F(설정)** — 서로·완료 트랙과 파일 무겹침.
