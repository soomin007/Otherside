...
2. **세이브 영속성 (중요):** 한 판 진행해 세이브를 만든 뒤, **빌드를 한 번 재업로드**하고 다시 열어
   "원정 이어가기"가 살아 있는지 확인. itch 는 업로드마다 서빙 경로가 바뀌어서 브라우저 저장소가
   유실될 수 있다는 보고가 있다. **유실되면 공개 전에 알릴 것** — 세이브 내보내기/가져오기 기능을
   먼저 넣고 공개하는 걸 권장.
