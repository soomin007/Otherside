...
**사용자 몫 (itch 계정 필요):**
- [x] itch.io 계정 생성 → 새 프로젝트 생성 (soominsnu)
- [x] §3 설정값 입력, §4 소개문 복붙, §5 이미지·테마 적용 (2026-07-26)
- [x] 빌드 zip 업로드 — 0.3 (언어 즉시 전환 포함)
- [x] 영어 설문 폼 개설 완료 (2026-07-26): https://forms.gle/B8qrsZJY7Fse3ybi6 — 소개문 §4 반영됨
- [x] 공개 전 검증 완료 — 세이브 영속성 ✅ · 폰 전체화면 가로 ✅ (2026-07-26) → **Public 전환됨**
...
- 페이지 URL 은 문서 상단에 기록됨. 주소를 짧게 치기 불편하면 `itch_업로드/QR_페이지.png`(폰 스캔용)를 쓴다.
  URL 슬러그 변경은 가능하지만 **기존 링크가 전부 깨진다(리다이렉트 없음)** — 공유가 시작된 뒤엔 바꾸지 않는다.
