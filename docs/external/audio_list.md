...
- **길이 줄이기(중요):** Suno v4.5+ 는 기본으로 길게(최대 ~8분) 뽑는다. **`Instrumental` 토글은 길이 제어가 안 된다.** 대신 Lyrics 를 **`Write` 모드로 두고 가사 없이 구조 태그만** 넣는다(단어가 없으니 그대로 인스트루멘탈, 대신 짧은 구조를 따라감).
  ```
  [Intro]
  [Main Theme]
  [Outro]
  [End]
  ```
  섹션 적게 + `[End]`/`[Fade out]` 일찍 두면 짧아진다. (`More Options` 슬라이더는 길이 제어가 아니다.) 스타일 칸의 `short, under 90 seconds, single loop` 은 보조(엄격하진 않음).
