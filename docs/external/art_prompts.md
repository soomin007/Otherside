...
| 55~57 | **막간(런 사이) 삽화 3종** (폭풍/고요/채비, 16:9) | §15 | ✅ 완료 (2026-07-12 삽입·`Interlude.gd` 배선 — 3장면 크로스페이드+켄 번즈+런 요약, lossy 0.8) |
| 58~62 | **사람 픽토그램 5종** (대원 갑/을·대장·낙오자·스러진 자리) | §16 | ✅ 완료 (2026-07-12 흰→투명 + `_밝음` 아이보리판 코드 파생(`ink_ivory.gd`). 배선: 지도 행렬 트레일·좌 칼럼 얼굴 줄·낙오자 마커 `Map.gd`) |
...
| 80~81 | **게임 로고**(제목 레터링) — 80 글자판=타이틀 화면, 81 행렬판(지평선+행렬)=오프닝 제목 카드 | §18 | ✅ **모두 C안 삽입 완료**(2026-07-18). 80 = 사용자 생성 C안 글자판(마젠타 크로마키). 81 = **합성판**(C안 글자판 + B안 구판의 지평선·행렬 스트립 — 생성 한도로 재생성 대신 합성, 두 화면 스크린샷 확인). 원본 arts_originals |

**01~81 전부 삽입 완료(2026-07-18).** 여유 생기면 81을 진짜 C안 행렬판으로 재생성해도 됨(§18 프롬프트 + 행렬 문장) — 현 합성판으로 충분하면 그대로. (가로 고정 게임 — 세로판 없음.)
...
## 15. 막간(런 사이) 삽화 (불투명 · 가로 16:9 · 1600×900 이상) — 3장, 하나씩 복사 (2026-07-11 추가)
...
> **하단 1/3 은 차분하게**(스크림도 깔림). 톤은 §7 오프닝·§13 엔딩과 같은 세트로 뽑을 것(같은 생성기·같은 세션).
> **게임은 가로 고정(2026-07-05 결정) — 모든 풀스크린 아트는 16:9 한 장만. 세로판 없음.**
> 파일명: `55_막간_폭풍`·`56_막간_고요`·`57_막간_채비`. 배선은 이미지 삽입 후 별도 작업.

### 55 폭풍 — 폭풍이 세상을 쓸어간다
```
A storytelling illustration, wide 16:9 landscape, a colossal wall of sandstorm sweeping in from the
right across a night desert, a small huddled desert town with low domes dark in the lower-left corner,
dust devouring the horizon and most of the sky, the center-bottom kept calm and dark. Muted sand and
sepia palette with deep indigo night tones, heavy dusty haze, painterly semi-realistic, low saturation,
ominous awe-filled mood, a full background scene. Absolutely no text, no words, no letters, no labels,
no watermark, no white background, no people.
```

### 56 고요 — 모래가 다시 가라앉았다
```
A storytelling illustration, wide 16:9 landscape, the desert at first pale dawn just after a great
sandstorm has passed, fine dust still settling in the air, smooth reshaped dunes, a half-buried broken
marker post and the last trace of an old path barely showing through fresh sand off to one side, hushed
and still, the center-bottom kept calm and open. Muted sand and sepia palette, thin dusty haze,
painterly semi-realistic, low saturation, hushed melancholic mood, a full background scene. Absolutely
no text, no words, no letters, no labels, no watermark, no white background, no people.
```

### 57 채비 — 다음 원정대가 떠날 채비를 한다
```
A storytelling illustration, wide 16:9 landscape, a small new expedition gathering at a desert town
gate before dawn on the left, a few hooded figures with worn packs around one warm lantern, seen from
behind and small, the open desert and a pale endless horizon stretching away to the right, the
...
### 삽입 완료 기록 (2026-07-18)
- **80 = C안 글자판 삽입 완료(2026-07-18 오후).** 마젠타 크로마키 실전 첫 통과 — 이때 도구 결함 2건을
  고침: ① despill 이 파랑만 보정해 마젠타의 빨강 성분이 남아 입자가 연어색 → 빨강/초록 비율(ρr)도
  학습해 초과분의 최대치로 보정 ② 생성물 극단 가장자리 심(seam)이 문턱을 넘어 점선 잔재+그림자로
  남음 → 키잉 전 사방 `EDGE_TRIM`(4px) 트림.
- **81 = 합성판 삽입 완료(같은 날).** 이미지 생성 한도로 C안 행렬판을 못 뽑아, C안 글자판(80) 아래에
  B안 구판의 지평선·행렬 스트립(y392~447, x119~1399)을 잘라 붙였다(사용자 제안). 오프닝 제목 카드에서
  로고 속 지평선이 배경 밤 풍경의 지평선과 자연스럽게 겹치는 것까지 확인. 여유 생기면 진짜 C안
  행렬판으로 교체 가능(선택).
- A안·B안 두 장 모두 채택(구판): **80 글자판 = 타이틀 화면**(키아트에 수평선이 이미 있어 지평선 없는 판),
  **81 행렬판 = 오프닝 제목 카드**(어두운 맨 배경 + 행렬이 슬라이드 서사를 받는다). 원본은 arts_originals.
- 검정→투명 변환 도구: `scripts/tools/black_key.gd` (alpha_key 의 자매 — 밝기 램프 언프리멀티플라이 +
  내부 구멍 메움 + O 속 구멍 재분류 + 크롭 + **그림자 굽기**(밝은 키아트 가독, 셰이더 금지라 에셋에 굽는다) +
  **광학 중심 정렬**(열/행 알파 밀도로 본체만 잡아 중앙 보정 — 한쪽으로 뻗는 뿌림이 정렬을 밀던 것,
  2026-07-18 사용자 지적 2건 반영) + 1600px 다이어트). 실행법은 파일 헤더.
- 임포트: lossy webp 0.8 + 밉맵(타이틀 띠에서 3배 축소되므로 앨리어싱 방지).
- 주제목(세계의 이름)이 나중에 정해지면 록업 위에 작게 얹는다 — 기획서 §7, backlog 참조.
