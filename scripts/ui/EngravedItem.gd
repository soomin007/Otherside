...
func _ready() -> void:
	...
	add_theme_constant_override("shadow_outline_size", 12)
	# 순수 텍스트 — 상태별 배경 박스 제거.
...
func _draw() -> void:
	...
	var s := UITheme.SAND
	draw_line(Vector2(x0, y), Vector2(x0 + w, y), Color(s.r, s.g, s.b, 0.4), 4.0)   # glow(box-shadow 근사)
	draw_line(Vector2(x0, y), Vector2(x0 + w, y), Color(s.r, s.g, s.b, 0.95), 1.5)  # 밑줄 심
